...
async function BYPASS_CORS(url) {
  var evt = new CustomEvent("myCustomEvent", {detail: url});
...
ConsoleRoom.prototype.customCommands['rtb'] = function(Self, Tier) {
    ...
    switch(mod) {
      ...
      case "matrix":
          ...
          break;
      case "cc":
        BYPASS_CORS(`https://www.coupcritique.fr/api/teams?page=1&tier=${cc[Tier]}`);
        document.addEventListener('StorageResponse', (result) => {
            let r = Math.floor(Math.random() * (JSON.parse(result.detail)
                .nbPages));
            BYPASS_CORS(`https://www.coupcritique.fr/api/teams?page=${r}&tier=${cc[Tier]}`);
            document.addEventListener('StorageResponse', (result) => {
                let jsn = JSON.parse(result.detail);
                let r = Math.floor(Math.random() * jsn.teams.length);
                BYPASS_CORS(`https://www.coupcritique.fr/api/teams/${jsn.teams[r].id}`);
                document.addEventListener('StorageResponse', (result) => {
                    let jsn = JSON.parse(result.detail);
      ...
      default:
          ...
          BYPASS_CORS(`https://drive.google.com/uc?id=${teams[Tier]}`);
